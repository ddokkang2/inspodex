...
## 0.3.42 (2025-12-16)
- 사용 흐름 가이드 강화: “스타일 찾기(디렉토리→검색/필터→카드 클릭)” 단계 표시 + 현재 단계 강조.
- 외부 검색 가이드 개선: 사이트 미선택 상태에서도 카드 클릭으로 곧바로 “검색” 단계로 전환, 검색 확장(선택) 영역을 상황에 따라 함께 강조.

## 0.3.43 (2025-12-16)
- 컬러 팔레트 디렉토리: 포즈처럼 전용 탐색 옵션(톤/온도) 점프 필터 추가(카운트 포함) + A‑Z/숫자 점프 숨김.
- 컬러 팔레트 “검색 확장” 프리셋 보강: 스와치/HEX, 시네마틱, 인테리어 추가.

## 0.3.44 (2025-12-16)
- 컬러 팔레트: 기존 A‑Z/숫자 점프를 유지한 채 톤/온도 옵션을 함께 제공(팔레트에서 초성 상태 초기화/무시로 인해 ‘재배열’처럼 보이던 체감 수정).

## 0.3.45 (2025-12-16)
- 컬러 팔레트: 나노바나나용 이미지 프롬프트 프리셋(12개) 추가 + 팔레트 카드 프롬프트에 HEX 5개 포함해 복사.

## 0.4.0 (2025-12-17)
- 컬러 팔레트 디렉토리 전면 교체: 10개 1차 분류 × 20개 세부 프리셋(총 200장)으로 재구성.
- 컬러 팔레트 스타일 찾기: 초성(A‑Z/숫자) 점프 제거, 1차 분류/세부 프리셋 점프 필터로 탐색.
- 컬러 팔레트 외부검색: Pinterest/Google 이미지만 제공, Coolors/Adobe Color 등은 “추천 링크”로 분리.
- 컬러 팔레트 프롬프트: 카테고리 맥락 문장을 자동 추가 + 프롬프트 프리셋(선택) 적용.

## 0.4.1 (2025-12-17)
- P0: 초기 로드 시 검색 패널 기본 열림 + 토글 라벨/aria-label을 “열기/닫기”로 정확히 반영.
- P0: 단계 가이드(1/2/3)에 완료(✓) 상태 표시 추가, 혼동을 주는 포커스/스포트라이트 애니메이션 제거(카드 원형 라이트 제거, 가이드 펄스 제거).

## 0.4.2 (2025-12-17)
- P1: 카드 클릭 시 외부 검색 입력 영역으로 자동 포커스/스크롤(모바일 우선).
- P1: “결과/전체” 카운터를 카드 그리드 상단으로 이동 + 필터 적용 배지 + 숫자 변경 애니메이션.
- P1: 적용된 필터(Active filters)를 결과 카운터 옆으로 이동해 항상 표시.
- P1: 컬러 팔레트 카드에 HEX 5개 표시 + 클릭 시 개별 HEX 복사(토스트 피드백).

## 0.4.3 (2025-12-17)
- P2: 카드 hover/active 시 확대·그림자·테두리 accent로 클릭 affordance 강화.
- P2: 태그 hover/focus 피드백 강화(토글 필터 사용성 개선).
- P2: 프롬프트 복사 팝업에 미리보기 추가 + 팔레트는 “HEX 5개 포함” 문구 표시.
- P2: 검색창 X 버튼은 입력 전 숨김, 입력 후 노출(실수 클릭 감소).

## 0.4.4 (2025-12-17)
- 성능: 카드 그리드 40개씩 점진 렌더링(스크롤 시 추가 로드)으로 초기 DOM 부하 감소.
- 접근성: 디렉토리/정렬/사이트/필터칩/점프바에 `role="radiogroup"` + `role="radio"`/`aria-checked` 적용, 카드 `aria-label`/`aria-pressed` 강화.
- UX: 스타일 검색 placeholder에 단축키 힌트(`/`, `Esc`)가 항상 유지되도록 통일.

## 0.4.5 (2025-12-17)
- Design: 점프 필터 “필터 해제” 버튼 추가 + 검색 확장 칩 선택 시 추가되는 토큰을 바로 미리보기로 표시.
- Design: 이미지 생성용 프롬프트를 태그 기반 핵심 특성(그리드/미니멀/브루탈/네온 등) 중심으로 더 구체화.
- Design: 썸네일 누락 시 스타일 템플릿 SVG fallback 제공(흐릿/깨짐 방지).

## 0.4.6 (2025-12-17)
- Character: 2D/3D/Pixel/Realistic/Toon 퀵필터 추가(탐색 기준 강화) + 필터 해제 버튼 연동.
- Character: 카드 클릭 시 한글+영문 키워드를 함께 적용해 한국어 검색 편의 개선.
- Character: 썸네일 fallback을 스타일별 변형(패턴/아이콘)으로 다양화해 중복 체감 완화.
- Character: 이미지 생성 프롬프트를 “실루엣/비율/렌더링” 중심으로 간소화.

## 0.4.7 (2025-12-17)
- Photo: 디렉토리 전환 시 A-Z/숫자 필터 잔존 문제 방지(사진 진입 시 초기화) + 사진/라이팅 추천 링크 섹션 추가.
- Photo: 이미지 프롬프트에 피사체/카메라/장면 요소를 포함해 실무 장면 생성에 맞게 보강.
- Palette: 세부 프리셋 “더보기/접기”로 스크롤 부담 완화 + HEX를 밝기 기준으로 정렬해 일관성 강화.
- Palette: 프롬프트 프리셋 적용 내용 미리보기 추가 + 팔레트 프롬프트 문체를 카테고리별로 다양화 + 태그 3개 규칙화.
- Pose: 카드 그리드 상단에 “카드섞기” 도구를 노출해 발견성 개선 + 포즈 검색어 ko+en 반영 + 포즈 프롬프트를 동작/각도별로 구체화.
